...
public class ProductView {
	...
	public JPanel displayProducts(JFrame frame) {
		...
		for (ProductVO vo : productList) {
			...
			lblPrice.setBackground(new Color(230,220,255));
		...
		public void mouseClicked(MouseEvent e) {
			...
			for(int i = 0; i<lblList.size(); i++) {
				if(eLbl == lblList.get(i)) {
					eLbl.setBackground(new Color(204, 204, 255));
	...
	ActionListener btnL = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			//투입버튼 클릭시
			//JTextfield에 입력된 금액과 현재 금액을 비교, 양수이상이면 제품과 거스름돈 출력
			//금액이 부족하면 부족하다고 출력하고 다시 금액을 입력할 수 있도록 함
			int money = Integer.parseInt(insertTf.getText());
			if(money >= vo.getPrice()) {
				ImageIcon icon = new ImageIcon("image/"+vo.getImageName()+".jpg");
				lblResult.setIcon(icon);
				JOptionPane.showConfirmDialog(frame, "거스름돈: "+(money-vo.getPrice())+"원");
				//lblResult.setText(vo.getName()+"제품 나옴, 거스름돈 "+(money-vo.getPrice())+"원");
			}else {
				JOptionPane.showConfirmDialog(frame, "금액이 부족합니다.");
